...
describe('Formatter', () => {
    ...
    describe('formatIndent', () => {
        it('formats conditional compile items with spaces around the keywords', () => {
            (0, chai_1.expect)(formatter.format((0, undent_1.undent) `
...
sub main()
    ...
    if true then
        ...
        it('removes whitespace between conditional compile symbol and keyword', () => {
            ...
            `, { insertSpaceAfterConditionalCompileSymbol: false })).to.equal((0, undent_1.undent) `
                sub Main(inputArguments as object)
                    #const SOME_CONST = true
                    #if true
                        print" one"
        ...
        it('reduces to single space between conditional compile symbol and keyword', () => {
            ...
            `, { insertSpaceAfterConditionalCompileSymbol: true })).to.equal((0, undent_1.undent) `
                sub Main(inputArguments as object)
                    # const SOME_CONST = true
                    # if true
                        print" one"
        ...
        it('removes whitespace if present in component scope variable declaration', () => {
            (0, chai_1.expect)(formatter.format(`m.a`)).to.equal(`m.a`);
            (0, chai_1.expect)(formatter.format(`m.foo`)).to.equal(`m.foo`);
            (0, chai_1.expect)(formatter.format(`m. a`)).to.equal(`m.a`);
            (0, chai_1.expect)(formatter.format(`m. foo`)).to.equal(`m.foo`);
            (0, chai_1.expect)(formatter.format(`m .a`)).to.equal(`m.a`);
            (0, chai_1.expect)(formatter.format(`m .foo`)).to.equal(`m.foo`);
        ...
        it('formats with optional chaining operators', () => {
            formatEqual(`m. alpha = m. alpha`, `m.alpha = m.alpha`);
            formatEqual(`m .beta = m .beta`, `m.beta = m.beta`);
            formatEqual(`m . charlie = m . charlie`, `m.charlie = m.charlie`);
            formatEqual(`print alpha . beta`, `print alpha.beta`);
            formatEqual(`doSomething(alpha . beta)`, `doSomething(alpha.beta)`);
            formatEqual(`doSomething(doSomethingElse( alpha . beta . charlie))`, `doSomething(doSomethingElse(alpha.beta.charlie))`);
            formatEqual(`value = alpha . beta + delta . charlie`, `value = alpha.beta + delta.charlie`);
            formatEqual(`value = 1 + (alpha . beta * (delta . charlie - 2) )`, `value = 1 + (alpha.beta * (delta.charlie - 2))`);
            formatEqual(`value = { alpha: beta . charlie }`, `value = { alpha: beta.charlie }`);
        });
    ...
    describe('break composite keywords', () => {
        ...
        it('handles edge cases', () => {
            let tokens = formatter['formatters']['compositeKeyword'].format([{
                    text: 'endfunction',
        ...
        it('adds spaces at proper locations when supposed to', () => {
            ...
            (0, chai_1.expect)(format(['endtry', brighterscript_1.TokenKind.EndTry])).to.equal('end try');
            (0, chai_1.expect)(formatter.format(`sub add()\n    if true then\n        a=1\n    elseif true then\n        a=1\n    endif\nendsub`, {
                compositeKeywords: 'split'
            })).to.equal(`sub add()\n    if true then\n        a = 1\n    else if true then\n        a = 1\n    end if\nend sub`);
        });
    ...
    function formatEqual(incoming, expected, options) {
        expected !== null && expected !== void 0 ? expected : (expected = incoming);
