...
class Runner {
    ...
    getBsfmtOptions(runnerOptions) {
        ...
        const parseErrors = [];
        const config = (0, jsonc_parser_1.parse)(contents.toString(), parseErrors);
        //if there were errors parsing the bsfmt.json, fail now
        if (parseErrors.length > 0) {
            throw new Error(`Error parsing "${bsfmtPath}": ${(0, jsonc_parser_1.printParseErrorCode)(parseErrors[0].error)}`);
        }
        return config;
    }
    normalizeArgs(args) {
        ...
        (_a = args.cwd) !== null && _a !== void 0 ? _a : (args.cwd = process.cwd());
        return args;
    }
}
